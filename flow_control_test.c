...
#include <stdio.h>
int main(void) {
	fprintf(stderr, "SKIP: POSIX-only test (non-blocking pipes)\n");
	return 0;
}
#else
/* Cycle 45: per-peer flow control via non-blocking writes + pending
 * buffer. Daemon with slow child (sh -c 'sleep 2; cat') ensures
 * pending buffer fills; test verifies second client connects
 * successfully during the slow child's pause (core invariant). */
...
static int run_client(uint16_t server_port, client_ctx_t *cctx,
	...
	picoquic_quic_t *quic = picoquic_create(
			4, NULL, NULL, NULL, "hq-test", NULL, NULL, NULL, NULL,
	...
	picoquic_cnx_t *cnx = picoquic_create_client_cnx(
			quic, (struct sockaddr *)&srv, picoquic_current_time(), 0,
			"test.example", "hq-test", client_stream_cb, cctx);
	if (cnx == NULL) {
		picoquic_free(quic);
		close(sock);
		return -1;
	...
	while (picoquic_current_time() < deadline) {
		...
		int rc = picoquic_prepare_next_packet(quic, now, buf,
				sizeof(buf), &send_len, &sto, &sfrom, &if_idx,
				&log_cid, &last);
		if (rc == 0 && send_len > 0) {
		...
		if (picoquic_get_cnx_state(cnx) == picoquic_state_ready) {
			...
			if (!sent) {
				(void)picoquic_add_to_stream(cnx, 0,
						...
						send_size, 1);
				sent = 1;
				deadline = now + 3ull * 1000 * 1000;
			}
		}
		if (cctx->stream_len >= send_size) {
			break;
		}
	...
	picoquic_free(quic);
	close(sock);
	int ok = ready && sent && cctx->stream_len >= send_size;
	return ok ? 0 : -1;
}
...
int main(void) {
	...
	/* Send 8 KiB from first client while slow child is sleeping. */
	uint8_t large_payload[8192];
	memset(large_payload, 'X', sizeof(large_payload));

	client_ctx_t cctx1 = { { 0 }, 0 };
	int cli1_rc = run_client(SERVER_PORT, &cctx1, sizeof(large_payload));
	EXPECT(cli1_rc == 0);
	EXPECT(cctx1.stream_len >= sizeof(large_payload));

	/* Drain daemon output to check for backpressure log. */
	char log[8192];
	size_t log_len = 0;
	drain_stdout(d.stdout_fd, log, sizeof(log), &log_len, 1000);
	EXPECT(strstr(log, "backpressured") != NULL);
