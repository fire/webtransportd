...
/* Simple WebTransport echo: just write received data back to client. */
static int wtd_wt_echo_callback(picoquic_cnx_t *cnx, uint8_t *bytes,
	...
	if (event == picohttp_callback_post_data || event == picohttp_callback_post_fin) {
		if (length > 0) {
			picoquic_add_to_stream(cnx, 0, bytes, length, 0);
		}
	}
	return 0;
}
...
static int cmd_server(const char *cert, const char *key, uint16_t port,
	...
	path_table[0].path_callback = wtd_wt_echo_callback;
